...
// Solo CSV: las cuentas por cobrar ya no entran por acá, las sincroniza la API
// (syncReceivables en lib/alegra-sync.ts) — un Excel viejo pisaría la foto fresca.
...
export async function importAlegraFiles(files: AlegraFile[]): Promise<ImportSummary> {
    ...
    for (const f of files) {
        // Los CSV de Alegra son Windows-1252 con separador ';'.
        const rows = parseCsv(new TextDecoder("windows-1252").decode(f.buffer))
