...
// Revertir un movimiento cargado por error. Sin confirmación: la acción ya es
// el arrepentimiento, y volver a preguntar solo agrega un paso. Lo que sí hace
// es aclarar en el aviso que queda registrado como corrección, para que nadie
...
export function UndoMovementButton({ movementId }: { movementId: number }) {
    ...
    function onClick() {
        startTransition(async () => {
            ...
            if (!res.ok) {
                toast.error("No se pudo revertir el movimiento", { description: res.error })
                return
            }
            setListo(true)
            toast.success("Movimiento revertido", {
                description: "Se registró el movimiento inverso para restablecer el stock.",
            })
            router.refresh()
        })
    }
    ...
    return (
        ...
        >
            ...
            {pending ? "Revirtiendo…" : listo ? "Revertido" : "Revertir"}
        </button>
    )
}
